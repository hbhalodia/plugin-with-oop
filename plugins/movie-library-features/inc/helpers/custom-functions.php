<?php
/**
 * Movie_Library features custom functions.
 *
 ...
 */
function movie_library_get_cache_key( $unique = '' ) {

	$cache_key = 'movie_library_cache_';
 ...
 */
function movie_library_get_cached_posts( $args ) {
	...
	$cache_key = movie_library_get_cache_key( $args );

	$cache  = new \Movie_Library\Features\Inc\Cache( $cache_key );
 ...
 */
function movie_library_get_template_content( $slug, $vars = [] ) {
 ...
 */
function movie_library_features_template( $template, $variables = [], $echo = false ) {

	$template_file = sprintf( '%1$s/templates/%2$s.php', MOVIE_LIBRARY_FEATURES_PATH, $template );
 ...
 */
function movie_library_get_data( $slug, $default = [] ) {

	$data_file = sprintf( MOVIE_LIBRARY_FEATURES_PATH . '/inc/data/%s.php', $slug );
 ...
 */
function movie_library_is_production() {
 ...
 */
function movie_library_is_mobile( $kind = 'any', $return_matched_agent = false ) {
